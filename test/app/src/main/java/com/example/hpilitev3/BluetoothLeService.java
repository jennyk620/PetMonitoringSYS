...
public class BluetoothLeService extends Service {
    ...
    public final static UUID UUID_SERVICE_SERIAL_PORT =
            UUID.fromString(SampleGattAttributes.UUID_SERVICE_SERIAL_PORT);
    public final static UUID UUID_CHARACTERISTIC_FIFO =
            UUID.fromString(SampleGattAttributes.UUID_CHARACTERISTIC_FIFO);
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                BluetoothGattCharacteristic ch = gatt.getService(UUID_SERVICE_SERIAL_PORT).getCharacteristic(UUID_CHARACTERISTIC_FIFO);
                gatt.setCharacteristicNotification(ch, true);

                BluetoothGattDescriptor descriptor = ch.getDescriptor(UUID.fromString(SampleGattAttributes.CLIENT_CHARACTERISTIC_CONFIG));
        ...
        public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status){
            BluetoothGattCharacteristic characteristic =
                    gatt.getService(UUID_SERVICE_SERIAL_PORT)
                            .getCharacteristic(UUID_CHARACTERISTIC_FIFO);
...
private void broadcastUpdate(final String action, final BluetoothGattCharacteristic characteristic) {
    ...
    final int type = characteristic.getWriteType();
    //     final int i = characteristic.getIntValue(BluetoothGattCharacteristic.FORMAT_SINT16, 0);
    final String str = characteristic.getStringValue(0);
    final String str_b = new String(data);
    Log.e("stringBuilder11", characteristic.getValue().toString());
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    private void broadcastUpdate(final String action, final BluetoothGattCharacteristic characteristic) {
        ...
        if(UUID_SERVICE_SERIAL_PORT.equals(characteristic.getUuid())) {
            final byte[] data = characteristic.getValue();
            if (data != null && data.length > 0) {
                //final StringBuilder stringBuilder = new StringBuilder(data.length);
                //for(byte byteChar : data)
        ...
        if (data != null && data.length > 0) {
            ...
            //Log.e("twentyfourbit0", stringBuilder.toString() + "");
            // bluno의 경우
            // new string(data),  new String(data, StandardCharsets.UTF_8), new String(data, StandardCharsets.US_ASCII),new String(data, StandardCharsets.ISO_8859_1)
            //Log.e("twentyfourbit1", new String(data));
        }
    }*/
    ...
    private void processData(byte[] value) {
        ...
        Read_data += data;
        if(data.contains("E")) { // 마지막 문자
            //Log.d("processData", Read_data);
            if(Read_data.contains("S")) {
